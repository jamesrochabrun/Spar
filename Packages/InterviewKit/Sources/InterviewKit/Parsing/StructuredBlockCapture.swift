...
public enum BuddyFence {
  ...
  public static func containsBuddyFence(_ text: String) -> Bool {
    text.contains("```\(question)") || text.contains("```\(eval)")
  }
}
...
public enum StructuredBlockCapture {
  ...
  /// Parses every buddy-question and buddy-eval fence in the message.
  /// The last buddy-eval fence wins (repair turns re-emit the block).
