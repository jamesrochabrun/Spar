...
struct ActiveFileView: View {
    ...
    var body: some View {
                ...
                if let onTogglePin = onTogglePin {
                    Button(action: onTogglePin) {
                        Image(systemName: isPinned ? "pin.fill" : "pin")
                            .foregroundColor(isPinned ? CodingBuddyChatRuntimeStyle.running : .secondary.opacity(0.6))
                            .font(.system(size: 12))
        ...
        .overlay(
            RoundedRectangle(cornerRadius: CodingBuddyChatRuntimeStyle.cardRadius)
                .strokeBorder(CodingBuddyChatRuntimeStyle.running.opacity(isPinned ? 0.3 : 0), lineWidth: 1)
                .animation(.easeInOut(duration: 0.2), value: isPinned)
        )
