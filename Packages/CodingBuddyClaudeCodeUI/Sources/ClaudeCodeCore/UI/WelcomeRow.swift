    ...
    Button(action: onSelect) {
      HStack(spacing: 8) {
        Image(systemName: worktree.isWorktree ? "arrow.triangle.branch" : "arrow.branch")
          .font(.caption)
          .foregroundColor(isSelected ? CodingBuddyChatRuntimeStyle.userText(for: colorScheme) : .secondary)
        
        VStack(alignment: .leading, spacing: 2) {
          Text(worktree.branch ?? "unknown")
            .font(.system(.caption, design: .monospaced))
            .foregroundColor(isSelected ? CodingBuddyChatRuntimeStyle.userText(for: colorScheme) : .primary)
          Text(URL(fileURLWithPath: worktree.path).lastPathComponent)
            .font(.system(.caption2, design: .monospaced))
            .foregroundColor(isSelected ? CodingBuddyChatRuntimeStyle.userText(for: colorScheme).opacity(0.8) : .secondary)
        }
        ...
        if isSelected {
          Image(systemName: "checkmark.circle.fill")
            .font(.caption)
            .foregroundColor(CodingBuddyChatRuntimeStyle.userText(for: colorScheme))
        }
      }
      .padding(.horizontal, 8)
      .padding(.vertical, 4)
      .frame(maxWidth: .infinity, alignment: .leading)
      .background(isSelected ? CodingBuddyChatRuntimeStyle.userBubble(for: colorScheme) : Color.clear)
