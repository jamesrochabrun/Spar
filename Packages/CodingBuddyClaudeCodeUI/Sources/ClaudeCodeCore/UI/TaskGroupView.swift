...
struct TaskGroupView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: CodingBuddyChatRuntimeStyle.Spacing.cardContentSpacing) {
      HStack(spacing: CodingBuddyChatRuntimeStyle.Spacing.headerDotSpacing) {
        taskStatusIndicator
        ...
        if let status = latestToolStatus, status.isExecuting {
          Text(EaselToolCardPresentation(toolUse: status.tool, toolResult: nil).title)
            .font(CodingBuddyChatRuntimeStyle.Typography.secondaryBody.weight(.medium))
            .foregroundStyle(CodingBuddyChatRuntimeStyle.running)
            .lineLimit(1)
      ...
      .padding(.horizontal, CodingBuddyChatRuntimeStyle.Spacing.taskHeaderHorizontal)
      .padding(.vertical, CodingBuddyChatRuntimeStyle.Spacing.taskHeaderVertical)
  ...
  private var taskStatusIndicator: some View {
    if latestToolStatus?.isExecuting == true {
      ...
      Image(systemName: "checkmark.circle.fill")
        ...
        .foregroundStyle(CodingBuddyChatRuntimeStyle.completedForeground(for: colorScheme, themeColors: appearanceSettings.themeColors))
        .frame(width: 14, height: 14)
        .accessibilityHidden(true)
    }
  }
