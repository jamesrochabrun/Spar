    ...
    VStack(spacing: 0) {
      ...
      if showingFileSearch {
        if let viewModel = fileSearchViewModel {
          ...
          .padding(.bottom, 8)
          .frame(maxWidth: CodingBuddyChatRuntimeStyle.maxContentWidth)
...
extension ChatInputView {
  ...
  private var sendButton: some View {
    Button(action: {
      ...
      Image(systemName: "arrow.up")
        ...
        .foregroundStyle(isTextEmpty ? CodingBuddyChatRuntimeStyle.tertiaryText(for: colorScheme) : CodingBuddyChatRuntimeStyle.userText(for: colorScheme))
        .frame(width: 30, height: 30)
        .background(
          isTextEmpty
            ? CodingBuddyChatRuntimeStyle.composerControlBackground(for: colorScheme)
  ...
  private var inputCornerRadius: CGFloat {
    min(CGFloat(uiConfiguration.inputCornerRadius), CodingBuddyChatRuntimeStyle.cardRadius)
  }
  ...
  private var inputBackground: AnyShapeStyle {
    ...
    return AnyShapeStyle(CodingBuddyChatRuntimeStyle.panelBackground(for: colorScheme))
  }
  ...
  private var inlineSearchBackground: AnyShapeStyle {
    ...
    return AnyShapeStyle(CodingBuddyChatRuntimeStyle.cardBackground(for: colorScheme))
  }
  
  /// Input area border
  private var inputBorder: some View {
    RoundedRectangle(cornerRadius: inputCornerRadius)
      .stroke(isDragging ? CodingBuddyChatRuntimeStyle.running : CodingBuddyChatRuntimeStyle.border(for: colorScheme), lineWidth: isDragging ? 2 : 1)
  ...
  private var textEditor: some View {
    ZStack(alignment: .center) {
      TextEditor(text: $text)
        ...
        .focused($isFocused)
        .font(.system(size: uiConfiguration.messageFontSize))
        .tint(CodingBuddyChatRuntimeStyle.inputTint(for: colorScheme))
