...
private struct EaselMarkdownCodeBlockView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      ScrollView(.horizontal, showsIndicators: true) {
        CodeText(code)
          ...
          .font(CodingBuddyChatRuntimeStyle.Typography.code(size: 12.5))
          .textSelection(.enabled)
          .padding(12)
    ...
    .clipShape(RoundedRectangle(cornerRadius: CodingBuddyChatRuntimeStyle.compactRadius, style: .continuous))
    .overlay(
      RoundedRectangle(cornerRadius: CodingBuddyChatRuntimeStyle.compactRadius, style: .continuous)
  ...
  private var header: some View {
    HStack(spacing: 8) {
      if let displayName = languageMapper.displayName(for: language) {
        Text(displayName)
          .font(CodingBuddyChatRuntimeStyle.Typography.code(size: 11.5))
...
private struct EaselMarkdownThemeColors {
  ...
  init(role: MessageRole, colorScheme: ColorScheme, themeColors: ThemeColors) {
    switch role {
    case .user:
      let userText = CodingBuddyChatRuntimeStyle.userMessageText(for: colorScheme, themeColors: themeColors)
    ...
    default:
      primaryText = .primary
      secondaryText = CodingBuddyChatRuntimeStyle.secondaryText(for: colorScheme, themeColors: themeColors)
      link = EaselDesignSystem.Palette.accentForeground(for: colorScheme)
      border = CodingBuddyChatRuntimeStyle.border(for: colorScheme, themeColors: themeColors)
      inlineCodeBackground = CodingBuddyChatRuntimeStyle.subtleCardBackground(for: colorScheme, themeColors: themeColors)
      codeHeaderBackground = CodingBuddyChatRuntimeStyle.cardBackground(for: colorScheme, themeColors: themeColors)
      codeBackground = CodingBuddyChatRuntimeStyle.subtleCardBackground(for: colorScheme, themeColors: themeColors)
